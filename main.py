...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplivativovendaskiv-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}' #precisa ser enviado em forma de json, essa formatação faz o dic virar string
        requisicao = requests.patch(f"https://aplivativovendaskiv-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.mudar_tela('ajustespage')

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplivativovendaskiv-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplivativovendaskiv-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
                mensagem_texto.text = "Vendedor adicionado com sucesso!"
                #adicionar banner de novo vendedor para acompanhar
                pagina_listavendedores = self.root.ids["listarvendedorespage"]
                lista_vendedores = pagina_listavendedores.ids["lista_vendedores"]
    ...
    def adicionar_venda(self):
        ...
        # preenchimento completo, vamos executar o codigo de adicionar venda
        if cliente and produto and unidade and preco and quantidade and (type(preco)==float) and (type(quantidade)==float):
            ...
            info = f'{{"cliente": "{cliente}", "foto_cliente": "{foto_cliente}", "produto": "{produto}", ' \
                   f'"foto_produto": "{foto_produto}", "data": "{data}", "preco": "{preco}", "unidade": ' \
                   f'"{unidade}", "quantidade": "{quantidade}"}}'

            requests.post(f"https://aplivativovendaskiv-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                          data=info)

            banner = BannerVenda(cliente=cliente, foto_cliente=foto_cliente,
                                         produto=produto, foto_produto=foto_produto,
                                         data=data, preco=preco, unidade=unidade,
            ...
            requisicao = requests.get(f"https://aplivativovendaskiv-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        #preencher todas as vendas
        requisicao = requests.get(f'https://aplivativovendaskiv-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
